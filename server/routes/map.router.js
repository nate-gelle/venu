...
const getMileage = (appointmentObject) => {
  return new Promise((resolve, reject) => {
    axios.get('https://maps.googleapis.com/maps/api/distancematrix/json?', {
      params: {
        units: 'imperial',
        origins: '44.977034,-93.174156|45.250413,-93.265450',
        destinations: venueObject.location,
        key: process.env.GOOGLE_MAPS_API_KEY || NULL,
      }
    }).then(response => {
      const mileage = response.data.rows;
      venueObject.forms[0].values[0].value = mileage[0].elements[0].distance.text;
      venueObject.forms[0].values[1].value = mileage[1].elements[0].distance.text;
      resolve(venueObject);
    }).catch(error => {
      console.log(error);
      reject(error);
...
//       for (let i = 0; i < appointments.length; i++) {
//         const marketAppointment = await getMileage(appointments[i]);
